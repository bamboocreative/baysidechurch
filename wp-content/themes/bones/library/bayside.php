...
function get_single_event(){
	...
	$event = array(
		...
		'ages' => get_field('age_range'),
		'register' => get_field('register_button_name')
...
function get_videos($category = '', $post_type = 'weekend', $tax = 'series'){
...
function get_single_video(){
	...
	if (is_numeric(get_field('video_id'))){
		...
		$service = 'youtube';
		$thumb = 'http://img.youtube.com/vi/' . get_field('video_id') . '/maxresdefault.jpg';
		
		$iframe = '<iframe src="//www.youtube.com/embed/' . get_field('video_id') . '?rel=0&showinfo=0" frameborder="0" allowfullscreen></iframe>';
	}
	
	$date = get_field('service_date');
	...
	$seriesquery = wp_get_post_terms(get_the_id(), 'series');
	...
	$weekend = array(
		...
		'provider' => $service,
		'date' => $date,
		'prettydate' => date('M j, Y', $date),
		'author' => $speaker,
		'author_twitter' => get_the_author_meta( 'twitter_username', $speaker['ID'] ),
		'message_notes' => get_field('message_notes'),
		'mp3' => get_field('mp3'),
		'custom_downloads' => get_field('custom_downloads'),
		'iframe' => $iframe,
		'series' => $series, 
		'series_header' => get_field('header_image', $series),
		'series_archive' => get_field('archive_image', $series),
		'worship' => get_field('worship'),
		'downloads' => get_field('custom_downloads'),
		'today' => $today
...
function add_to_context($data){
    ...
    $campuses = wp_get_sites();
	foreach($campuses as $campus){
	
		if($campus['blog_id'] != get_current_blog_id() && $campus['blog_id'] != 1){
			$campusDetails[] = get_blog_details( $campus['blog_id'] , true );
		}
